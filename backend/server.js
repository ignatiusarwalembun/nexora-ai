...
app.post('/api/config/ai',requireAuth,(req,res)=>{try{const ws=workspaceOf(req);const{apiKey,model,embeddingModel,assistantName,systemPrompt}=req.body||{};if(!model?.trim())throw new Error('Model wajib diisi.');updateConfig(ws,config=>{if(String(apiKey||'').trim())config.ai.apiKey=seal(String(apiKey).trim());config.ai.model=String(model).trim();config.ai.embeddingModel=String(embeddingModel||'text-embedding-3-small').trim();config.ai.assistantName=String(assistantName||'Maba Assistant').trim();config.ai.systemPrompt=String(systemPrompt||config.ai.systemPrompt).trim();});res.json({ok:true,config:publicConfig(ws).ai});}catch(err){res.status(400).json({ok:false,error:err.message});}});
...
app.listen(PORT,()=>{console.log(`\n[MABA BUSINESS] Phase 4 running → http://localhost:${PORT}\n`);startAllTelegramPolling();});
